...
##import data
curve <- read.csv("C:/Users/John/Desktop/area curve.csv")
...
model5 = lme(curve ~ site + task + type, 
             data = noout, 
             method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
...
#adding interaction
model6 = lme(curve ~ site + task*type,
             data = noout,
             method = "ML",
             na.action = "na.omit",
             random = ~1|partno)
...
with(noout,tapply(curve, list(type,task,site), mean,na.rm=T))
hist(noout$curve)
...
scatter = ggplot(graphdata, aes(fitted, curve))
scatter +
  theme + 
  geom_point() +
...
##descriptives
tapply(noout$curve, list(noout$type, noout$task), mean)
tapply(noout$curve, list(noout$type, noout$task), sd)
tapply(noout$curve, list(noout$type, noout$task), length)
