...
model1 = gls(RT ~ 1, data = noout, method = "ML", 
             na.action = "na.omit")
summary(model1)
##random intercept only model
model2 = lme(RT ~ 1, data = noout, method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model2)
anova(model1,model2)
...
##adding task
model3 = lme(RT ~ task, data = noout, method = "ML", 
              na.action = "na.omit",
              random = ~1|partno)
summary(model3)
##adding task and type 
model4 = lme(RT ~ task + type, data = noout, method = "ML", 
              na.action = "na.omit",
              random = ~1|partno)
summary(model4)
#adding interaction
model5 = lme(RT ~ task*type,data = noout,method = "ML",
              na.action = "na.omit",
              random = ~1|partno)
summary(model5)
##compare models
...
summary(LDTmodel)
levels(LDT$type)
LDT$type = factor(LDT$type,
                  levels = c("LH","HL","UR","NW"),
                  labels = c("LH","HL","UR","NW"))
LDTmodel2 = lme(RT ~ type, data = LDT, method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LDTmodel2)
LDT$type = factor(LDT$type,
                  levels = c("HL","LH","UR","NW"),
                  labels = c("HL","LH","UR","NW"))
LDTmodel3 = lme(RT ~ type, data = LDT, method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LDTmodel3)
...
summary(LSTmodel)
levels(LST$type)
LST$type = factor(LST$type,
                  levels = c("LH","HL","UR","NW"),
                  labels = c("LH","HL","UR","NW"))
LSTmodel2 = lme(RT ~ type, data = LST, method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LSTmodel2)
LST$type = factor(LST$type,
                  levels = c("HL","LH","UR","NW"),
                  labels = c("HL","LH","UR","NW"))
LSTmodel3 = lme(RT ~ type, data = LST, method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LSTmodel3)
