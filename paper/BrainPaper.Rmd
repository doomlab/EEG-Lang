...
##Priming in the Brain
  ...
  @Rolke2001 used the attention blink rapid serial visual presentation (RSVP) paradigm, in which participants identified target words within a stream of distractor words presented in a different color. By selecting items via specifying the row and column within a matrix, participants identified the target word they had previously seen. These studies compare to masked priming, and show automatic activation of semantic information even when targets were missed [@Rolke2001]. 
  
  Letter search tasks also  reduce semantic priming by focusing attention on the lexical level instead of a feature meaning level [@Friedrich1991].  In this task, participants are asked to determine if cue and target words contain a specific letter presented.  @Stolz1999 stipulate that this eliminated or reduced priming indicates non-automatic semantic priming. However, it is also important to note that @Tse2007 did yield evidence that letter search primes produced semantic priming for low-frequency targets, albeit not for high-frequency targets.   In @Smith2001 letter search and lexical decision combined study, they found that the letter search task eliminated semantic priming when compared to unrelated word pairs and the lexical decision task.  However, @Mari-Beffa2005 found ERP evidence for semantic processing of the prime word during letter search tasks with the attenuation of the N400.
  
##Association
  From a theoretical standpoint, the relation between associative and semantic processing follows a deep line of research.   Associative word pairs are words that are linked in one’s memory by contextual relationships, such as basket and picnic [@Nelson2004].  Another example would be a word pair like alien and predator, which would be associatively linked for Americans due to the movies and popular culture. Semantic word pairs are those linked by their shared features and meaning, such as wasp and bee [@Buchanan2014; @McRae2005].
  
  Associative and semantic relationships between words are experimentally definable by the use of normed databases. @Maki2004 took the online dictionary, WordNet [@Fellbaum1998], and used software by @Patwardhan2003 to create a database of words displaying the semantic distance between individual words. This database displays the relatedness between two words by measuring how semantically close words appear in hierarchy, or the JCN [@Jiang1997]. JCN measures the word pairs' informational distance from one another, or their semantic similarities. Therefore, a low JCN score demonstrates a close semantic relationship.  Additionally, we can use a measure of semantic feature overlap to examine the semantic relatedness between word pairs (McRae, Vinson, Buchanan???), and this measure is factorally related to JCN as a semantic measure [@Maki2008].  Another useful database, created by @Nelson2004, is centered on the associative relationships between words.  Participants were given cue words and asked to write the first word that came to mind. These responses were asked of and averaged over many participants.  The probability of a cue word eliciting the target word is called the forward strength (FSG).  For example, when participants are shown the word lost, the most common response is found, which has a FSG of .75 or occurs about 75% of the time.  
...
#Results
```{r area under curve MLM, eval=FALSE, include=FALSE}
##import data
...
model1 = gls(curve ~ 1, data = noout, method = "ML", 
             na.action = "na.omit")
summary(model1)
##random intercept only model
model2 = lme(curve ~ 1, data = noout, method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model2)
anova(model1,model2)
...
##adding site
model3 = lme(curve ~ site, data = noout, method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model3)
##adding site and task 
model4 = lme(curve ~ site + task, data = noout, method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model4)
##adding site, task, type
model5 = lme(curve ~ site + task + type, data = noout, method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model5)
#adding interaction
model6 = lme(curve ~ site + task*type,data = noout,method = "ML",
             na.action = "na.omit",
             random = ~1|partno)
summary(model6)
##compare models
...
LDTmodel = lme(curve ~ site + type, data = LDT, method = "ML", 
               na.action = "na.omit",
               random = ~1|partno)
summary(LDTmodel)
levels(LDT$type)
...
LDTmodel2 = lme(curve ~ site + type,data = LDT,method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LDTmodel2)
LDT$type = factor(LDT$type,
...
LDTmodel3 = lme(curve ~ site + type,data = LDT,method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LDTmodel3)

LSTmodel = lme(curve ~ site + type,data = LST,method = "ML",
               na.action = "na.omit",
               random = ~1|partno)
summary(LSTmodel)
LST$type = factor(LST$type,
                  levels = c("LH","HL","UR","NW"),
                  labels = c("LH","HL","UR","NW"))
LSTmodel2 = lme(curve ~ site + type,data = LST,method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LSTmodel2)
LST$type = factor(LST$type,
                  levels = c("HL","LH","UR","NW"),
                  labels = c("HL","LH","UR","NW"))
LSTmodel3 = lme(curve ~ site + type,data = LST,method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LSTmodel3)
...
##EEG Data
```{r Peak MLM, eval=FALSE, include=FALSE}
##import data
...
#########################################MLM
##intercept only model
model1 = gls(peak ~ 1,  data = noout,  method = "ML", 
             na.action = "na.omit")
summary(model1)
##random intercept only model
model2 = lme(peak ~ 1,  data = noout,  method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model2)
anova(model1,model2)
##significant...on to fixed effects
##adding site
model3 = lme(peak ~ site, data = noout, method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model3)
##adding site and task 
model4 = lme(peak ~ site + task, data = noout, method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model4)
##adding site, task and type 
model5 = lme(peak ~ site + task + type, data = noout, method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model5)
#adding interaction with site as covariate
model6 = lme(peak ~ site + task*type,data = noout,method = "ML",
             na.action = "na.omit",
             random = ~1|partno)
summary(model6)
##compare models
...
LDT = subset(noout, task == "ldt")
LST = subset(noout, task == "lst")
LDTmodel = lme(peak ~ site + type, data = LDT, method = "ML", 
               na.action = "na.omit",
               random = ~1|partno)
summary(LDTmodel)
levels(LDT$type)
LDT$type = factor(LDT$type,
                  levels = c("LH","HL","UR","NW"),
                  labels = c("LH","HL","UR","NW"))
LDTmodel2 = lme(peak ~ site + type, data = LDT, method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LDTmodel2)
LDT$type = factor(LDT$type,
                  levels = c("HL","LH","UR","NW"),
                  labels = c("HL","LH","UR","NW"))
LDTmodel3 = lme(peak ~ site + type, data = LDT, method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LDTmodel3)

LSTmodel = lme(peak ~ site + type, data = LST, method = "ML", 
               na.action = "na.omit",
               random = ~1|partno)
summary(LSTmodel)
levels(LST$type)
LST$type = factor(LST$type,
                  levels = c("LH","HL","UR","NW"),
                  labels = c("LH","HL","UR","NW"))
LSTmodel2 = lme(peak ~ site + type, data = LST, method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LSTmodel2)
LST$type = factor(LST$type,
                  levels = c("HL","LH","UR","NW"),
                  labels = c("HL","LH","UR","NW"))
LSTmodel3 = lme(peak ~ site + type, data = LST, method = "ML",
                na.action = "na.omit",
                random = ~1|partno)
summary(LSTmodel3)
...
```{r Latency MLM, eval=FALSE, include=FALSE}
##import data
...
zscore = unsplit(lapply(split(longdata$latency, list(longdata$partno, longdata$task)),
                        scale),list(longdata$partno, longdata$task))
summary(abs(zscore) < 2.3585) ##4
noout = subset(longdata, abs(zscore) < 2.3585) ##exclude outliers

#########################################MLM
##intercept only model
model1 = gls(latency ~ 1, data = noout, method = "ML", 
             na.action = "na.omit")
summary(model1)
##random intercept only model
model2 = lme(latency ~ 1, data = noout, method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model2)
anova(model1,model2)
##significant...on to fixed effects
##adding site
model3 = lme(latency ~ site,  data = noout,  method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model3)
##adding site and task 
model4 = lme(latency ~ site + task, data = noout,  method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model4)
##adding site, task, type 
model5 = lme(latency ~ site + task + type,  data = noout,  method = "ML", 
             na.action = "na.omit",
             random = ~1|partno)
summary(model5)
#adding interaction
model6 = lme(latency ~ site+ task*type,data = noout,method = "ML",
             na.action = "na.omit",
             random = ~1|partno)
summary(model6)
##compare models
